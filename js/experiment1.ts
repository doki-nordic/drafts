...
function* tokenize(input: string) {
    ...
    while ((groups = regexp.exec(input)?.groups as (TokenRegExpGroups | undefined))) {
        ...
        if (groups.newLine !== undefined) {
            yield new TokenBase(TokenType.newLine, position, groups.whitespace, groups.newLine) as Token;
        } else if (groups.float !== undefined) {
            yield new TokenBase(TokenType.floating, position, groups.whitespace, groups.float) as Token;
        } else if (groups.integer !== undefined) {
            yield new TokenBase(TokenType.integer, position, groups.whitespace, groups.integer) as Token;
        } else if (groups.comment !== undefined) {
            yield new TokenBase(TokenType.comment, position, groups.whitespace, groups.comment) as Token;
        } else if (groups.operator !== undefined) {
            yield new TokenBase(TokenType.operator, position, groups.whitespace, groups.operator) as Token;
        } else if (groups.string !== undefined) {
            yield new TokenBase(TokenType.string, position, groups.whitespace, groups.string) as Token;
        } else if (groups.char !== undefined) {
            yield new TokenBase(TokenType.character, position, groups.whitespace, groups.char) as Token;
        } else if (groups.identifier !== undefined) {
            yield new TokenBase(TokenType.identifier, position, groups.whitespace, groups.identifier) as Token;
        } else if (groups.unknown !== undefined) {
            yield new TokenBase(TokenType.unknown, position, groups.whitespace, groups.unknown) as Token;
        } else if (groups.endOfText !== undefined) {
            yield new TokenBase(TokenType.end, position, groups.whitespace, '') as Token;
            return;
        } else {
            break;
        }
...
export class Tokenizer {
    ...
    constructor(input: string) {
        ...
        this.endToken = new TokenBase(TokenType.end, input.length, '', '') as Token;
    }
